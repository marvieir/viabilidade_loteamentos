...
```
Concluídas (validadas):  1 → … → 4 → 4.1 → 4.2 → 5 → 6 → 7 → 8 → 9 → 9.1 → 9.4 → 9.5 → 9.6 → 9.7 → 9.8 → 9.9 → 9.10
Próximas (nesta ordem):  pórtico de entrada (ancorado na via-tronco curva da 9.9) · refino fino do traçado (cul-de-sacs, praças, hierarquia de ramos) · Custos (SINAPI/SICRO) · Técnica/Operacional/Mercadológica/Política · Estadual
...
| 2026-06-17 | **Fase 9.8 CONCLUÍDA — Urbanismo: malha enxuta (poda de stubs + grade por ilha + rótulo da restrição).** Corrige a causa-raiz que o diagnóstico da 9.7 isolou: numa gleba **irregular/recortada**, a malha gerada sobre o **retângulo envolvente** virava **stubs** (cacos de via que não servem lote) → viário inflava para **26%** e os lotes caíam para **41** (a hierarquia tronco 21 m / local 8 m estava **correta**; o problema era a **forma**). Três correções determinísticas (§2 — IA propõe eixos; Python poda e mede, nenhuma decisão do LLM): **(1) MALHA POR ILHA** — `gerar_layout` itera `_componentes(aprov)` e gera a malha de cada ilha **recortada A ELA** (`_eixos_da_ilha` explode as retas da grade nos **segmentos dentro da ilha**, não no bbox da gleba); duas massas separadas por mata são **2 ilhas conexas**, não 1 desconexa (`conexo_por_ilha`). **(2) PODA DE STUBS** (`podar_stubs`) — o viário passa a ser a **rede de fronteiras INTERNAS entre faces** (`unary_union(face.boundary) − perímetro da ilha`): mantém a parte ÚTIL de cada eixo (que dá frente a dois blocos) e descarta os trechos pendurados/cacos que o recorte deixou; **passo único e estável** (as faces saem de `polygonize` com todos os eixos — um dangling não forma face —, então não há cascata). A **via-tronco proposta pela IA é PROTEGIDA** da poda (é a espinha; a sinuosidade é a 9.9); a central auto-promovida (sem IA) é podada como ramo local. A área dos stubs deixa de ser via → vira face (re-loteada/verde), invariância preservada. **(3) RÓTULO DA RESTRIÇÃO** — `_aproveitavel_wgs` agora devolve a **restrição recortada** (mata/declividade/APP que o motor não loteou) + origem; `geojson_do_layout` expõe `restricao_recortada` (geometria + `origem` + rótulo "não-edificável — ver cards Ambiental/Vegetação/Declividade") p/ o mapa **demarcar** em vez do "clarão". **Reuso intacto:** `_subdividir_quadra` + clamp/diretrizes (9.4), `lotes_features` (9.5), verde reservada/sobra (9.6), institucional/clube formados (9.7), medição. **Critérios verdes (calibrados no experimento do diagnóstico, offline):** gleba recortada (mata diagonal) **viário 26%→14,9%**, vendável **47%→48,5%**, **lotes 62→83** (>41), `ilhas=2`/`conexo_por_ilha`, `stubs_podados≥1`, nenhum trecho de via < 180 m² (o caco de 149 m² sumiu); **caixa limpa intacta** (~15%, conexa, ~64 lotes, 0 stubs — a poda não degrada o que já estava bom); clamp `fora_da_faixa==0`, retalho ≤1,5%, invariância ±0,5 m², determinismo, §1-A. **Recalibração (critério 9 da spec — valores de layout mudam):** `test_fronteira_programa_sem_numero` afere agora `areas_verdes_reservada is None` (nada RESERVADO; a sobra de ponta do loteamento pode ir ao verde total — leftover real, não default inventado). **8 testes novos (`test_urbanismo_poda.py`) · suíte 326 + 4 skip · regressão 1…9.7 zero · `tsc`/`next build`/vitest limpos.** Front: `MapaLeaflet`/`overlays` desenham `urb_restricao` (hachura escura "não-edificável"); `CardUrbanismo` mostra o diagnóstico de **ilhas conexas + cacos podados**. **Expectativa honesta (registrada):** a malha fica **enxuta e fiel à forma** (viário de volta a ~15%, sem cacos, restrição demarcada), mas o traçado **continua reto/angular** — a **sinuosidade** (via-tronco curva acompanhando o relevo) é a **Fase 9.9**, e dependerá de a IA **de fato propor** o esqueleto sinuoso (o diagnóstico mostrou que hoje o preset vem com esqueleto vazio → cai na grade reta; a 9.9 embute a verificação de que o produto chama o modelo p/ o esqueleto). `docs/fase-9.8-malha-enxuta-poda.md`. Fora de escopo: sinuosidade (9.9), pórtico, render artístico, otimização do traçado, custos. **Nota: a frente-para-via de ~50% dos lotes na caixa (linha do meio de blocos de 3 fileiras) é PRÉ-EXISTENTE da 9.7 — fora do escopo desta fase, candidata a refino junto da 9.9.** | 9.8 |
| 2026-06-18 | **Fase 9.9 CONCLUÍDA — Urbanismo: traçado SINUOSO (a IA propõe eixos curvos; o Python materializa contornando o íngreme).** Tira a grade reta e dá ao estudo a cara orgânica do urbIA, nas pontas loteáveis (o diagnóstico fechou que São Roque é majoritariamente encosta ≥30% → ~43 lotes é o número honesto; vegetação 7%, 0% dura). Ataca as DUAS causas da grade reta: **(1) o esqueleto vinha VAZIO** — diagnóstico: a tool/`_INSTRUCAO` marcavam o esqueleto como "OPCIONAL/omitível", o preset nunca o setava e, com o LLM fora, caía no preset. Correção: o prompt/tool schema agora **exige** via-tronco SINUOSA (polilinha ≥4 vértices, 0..1, contornando o íngreme) + ramos para arquétipo não-grelha; o gerador marca `esqueleto_origem="llm"`. **Se vier vazio/inválido, NÃO cai em grade silenciosa** — `gerar_layout` gera por ilha uma **espinha sinuosa de fallback explícito** (`_espinha_sinuosa`: ondulação senoidal ao longo do eixo maior da ilha, `esqueleto_origem="fallback_curva"`, `esqueleto_vazio=true`); a grelha (baixa) segue ortogonal por intenção (`origem="grade"`). **(2) materializar a curva** — `_eixos`/`_curva_suave` suavizam a polilinha (Catmull-Rom amostrado) em curva real (aceita `[[x,y],...]` ou `{"pontos":[...]}`); `construir_malha` usa o eixo curvo como TRONCO. Para o viário não estourar o teto: a via-tronco curva usa a **largura da via principal** (~14 m, não a coletora de 21 m da grelha), e a grade local **substitui** a linha paralela mais próxima da tronco (evita via dupla, sem dropar a família). **Contorno do íngreme por construção**: o `∩ ilha` (ilhas já são `aprov − restrição`) garante `via ∩ restricao_recortada == ∅`. **Correção de invariância (curva parte face em vários pedaços):** o miolo passou a capturar TODOS os componentes de `face − ruas` (não só o maior) — senão a área dos demais sumia (gap de 5,3% → 0). **Reuso intacto:** malha por ilha + poda (9.8), faces/institucional/clube (9.7), `_subdividir_quadra`+clamp (9.4), `lotes_features` (9.5), separação verde (9.6), rótulo da restrição (9.8). **Fronteira §2 (o ponto sensível):** o LLM fornece **só a geometria dos eixos** (a curva); nenhuma área/contagem/m² de via vem dele — Python suaviza, recorta e mede. `sinuosidade_media` (razão curva/reta) é indicador de APRESENTAÇÃO, não entra em área. **8 critérios verdes (calibrados no experimento do diagnóstico, offline):** esqueleto não-vazio + `origem` llm/fallback (nunca grade silenciosa em arquétipo curvo), `sinuosidade_media` **1,16 (caixa) / 1,35 (recortada)** > 1,1 + `eixos_curvos`, `via ∩ restrição == ∅`, viário **16,5%** ≤18% com lotes **~79 estáveis** (9.8 dava 83) e vendável **48,3%** (≈48,5% da 9.8), conexo por ilha, clamp `fora_da_faixa==0`, invariância ±0,5 m² / líquida≈aprov, §1-A regex. **Recalibrações (critério 8 — valores de layout mudam por traçado curvo):** `test_esqueleto_invalido` (×2) afere agora o **fallback de curva** (origem=fallback_curva, eixos simples) em vez de `not centerlines`; hierarquia do tronco no sinuoso = via principal (`tronco_m ≥ local_m`, não ≥21); fidelidade do lazer aceita atendido **ou** atenção. **8 testes novos (`test_urbanismo_sinuoso.py`); stub de LLM devolve polilinha curva → origem llm · suíte 334 + 4 skip · regressão 1…9.8 zero · `tsc`/`next build`/vitest limpos.** Front: as vias já vêm curvas na malha (`arruamento`); `CardUrbanismo` ganha o chip **"traçado sinuoso (curva X · IA/padrão)"** e a nota de traçado curvo aproximado. **Expectativa honesta (registrada):** primeira versão da sinuosidade — curvas suaves e fiéis ao relevo, contornando o íngreme; refino fino (cul-de-sacs, praças de retorno, hierarquia de ramos) é iteração futura. Como metade da gleba é não-edificável, o desenho ocupa só as pontas — isso é o correto, não limitação. `docs/fase-9.9-tracado-sinuoso.md`. **Depois desta: o pórtico de entrada**, que finalmente tem uma via-tronco curva onde ancorar. Fora de escopo: pórtico, refino executivo do traçado, render artístico, mudança de recorte/declividade/nº de lotes (43 é honesto). | 9.9 |
| 2026-06-18 | **Fase 9.10 CONCLUÍDA — Ponte de reconciliação (teto teórico × estudo realista).** Não é urbanismo nem bug — é **coerência entre abas**: o mesmo sistema mostrava **120 lotes** no Aproveitamento e **51** no Urbanismo para a mesma gleba; ambos corretos (medem perguntas diferentes), mas isolados pareciam contradição. **PURO texto/apresentação — ZERO mudança de cálculo:** `lotes_teto` (120) e `n_lotes` do Urbanismo (51) **idênticos**. Cada card passa a **rotular o seu número e citar o outro**. Diagnóstico que fundou a fase: os dois partem da MESMA base física (~58.920 m²); divergem por **duas premissas** — doação **20% (piso legal)** vs **46,7% (desenhada)** e lote **÷360 (mínimo legal)** vs **~500 (perfil real)** — mais a perda geométrica do traçado sinuoso (9.9). `core/aproveitamento.reconciliacao_aproveitamento` e `core/urbanismo_medida.reconciliacao_urbanismo` montam o bloco `reconciliacao` (premissas JÁ calculadas, só serializadas + `leitura` interpolada §1-A: "teto/estimativa/estudo/verificar", regex sem "cabem N"). **Referência cruzada SÓ exibição (sem acoplamento de cálculo):** o Aproveitamento busca o `n_lotes` do último snapshot de urbanismo da sessão (`ref_estudo_massa`); o Urbanismo recomputa o teto pela MESMA `cenario_diretriz` (mesma fórmula/cenário, físico idêntico) para `ref_teto_regulatorio` — nenhuma aba usa o número da outra em conta própria. **Degradação honesta:** sem a outra aba, mostra o seu número + convite a rodar a outra (não inventa). **8 critérios verdes:** números idênticos ao atual (a ponte só ECOA), Aproveitamento cita o estudo + a razão (lote/doação maiores), Urbanismo cita o teto, **texto interpolado** (mudar gleba muda os números — não hardcoded), degradação honesta, §1-A sem promessa, sem acoplamento. **8 testes (`test_reconciliacao.py`) + recalibração de `test_aditivo_nao_regride_aproveitamento` (a única diferença com/sem urbanismo é a ponte citar o estudo — números intactos) · suíte 342 + 4 skip · `tsc`/`next build`/vitest limpos.** Schemas: `RefCruzadaOut` + `Reconciliacao{Aproveitamento,Urbanismo}Out`. Front: caixa âmbar "Teto teórico × estudo de massa" (Aproveitamento) e "Estudo de massa × teto regulatório" (Urbanismo), com micro-resumo de faixa *"~120 (teto legal) → ~51 (estudo realista)"*. Mata a estranheza da divergência e dá ao usuário a **dupla-visão** (teto possível + piso realista) que uma triagem deve ter (§1-A). `docs/fase-9.10-ponte-reconciliacao.md`. Fora de escopo: mudar a fórmula do teto, aposentar o teto, geometria. **Depois desta: o pórtico.** | 9.10 |
