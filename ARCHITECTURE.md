...
```
Concluídas (validadas):  1 → 1.5 → 1.7 → 2.1 → 2.2 → 2.3 → 1.8 → 2.5 → 3
Próximas (nesta ordem):  1.6* → 3.5 → 4 → 5 → 6 → 7+
```
...
- **3.5 (próxima) — Conformidade urbanística** — consome o **perfil municipal que a 1.8 cria**
  (lote legal/doação/usos por zona); consumo puro, sem dado novo. Era a antiga "Fase 3".
...
12. **Fase 3.5 — Conformidade urbanística** (a especificar; era a antiga "Fase 3"). Confronta
    a gleba com os índices da LUOS que a 1.8 extraiu (`frente_min`/`ca`/`taxa_ocupacao`/
    `doacao_split`). Consumo puro, sem dado novo. Esboço em `docs/handoff-fase-3-pedido-spec.md`.
13. Fase 4 — Financeira (consome lotes do motor).
