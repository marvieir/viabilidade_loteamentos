...
```
Concluídas (validadas):  1 → … → 4 → 4.1 → 4.2 → 5 → 6 → 7 → 8 → 9 → 9.1 → 9.3
Próximas (nesta ordem):  Apresentação/traçado orgânico (Nível 1+2) · Custos (SINAPI/SICRO) · Técnica/Operacional/Mercadológica/Política · Estadual
                                                          (9.2 substituída pela 9.3; 1.6* parado: falta amostra)
```
