...
```
Concluídas (validadas):  1 → 1.5 → 1.7 → 2.1 → 2.2 → 2.3 → 1.8 → 2.5 → 3 → 3.5 → 4
Próximas (nesta ordem):  1.6* → 5 → 6 → 7+
```

- **1.6\*** — adaptador de topografia/CAD; segue **bloqueada** até reunir 3–5 arquivos reais.
- **3 — Jurídica documental** ✅ — pré-análise dominial (matrícula/certidões): ônus/averbações
  via extração assistida (reusa o motor da 1.8, sem chave nova) + cross-check de área + roll-up
  de risco. Não altera o número do aproveitável.
- **3.5 — Conformidade urbanística** ✅ — checklist da (zona, modalidade) contra a gleba com os
  índices da 1.8 que não entram no número (frente/CA/TO/split da doação). Consumo puro.
- **4 — Financeira** ✅ — fluxo de caixa do empreendimento (VGV, custos, tributo, fluxo mês a
  mês, exposição, margem). Aritmética pura; lotes do caso-base pela regra §3.1.
- **5 (próxima) — Econômica** — VPL/TIR/TMA/payback sobre o fluxo que a 4 padronizou.
  **A especificar:** pedido de spec em `docs/handoff-fase-5-pedido-spec.md`.
...
8. **Fase 2.2 — Área verde (cobertura vegetal)** ✅. ESA WorldCover 10 m (`/vsicurl`, sem login);
   ...
   de restrições, teto de lotes, vias/doação removidas). **Modo automático por gleba** (default):
   escolhe o tile do COG público pela posição — qualquer KMZ, **sem recorte manual**. Validada ao
   vivo (Terreno_Cachoeira **e São Roque**).
9. **Fase 2.3 — Severidade do verde** ✅. Separa **verde-em-APP/UC = restrição dura**
   (proibição legal) de **verde genérico = a verificar** (depende de laudo). Reusa as camadas
   da 2.1 (interseção geométrica a mais); não troca o WorldCover. Mantém o verde descontado do
   aproveitável (conservador), mas para de tratar uma APP de rio e um pasto arborizado igual.
   Ganchos: composição por classe (árvores vs arbustiva); série temporal (futuro).
10. **Fase 2.5 — Declividade via DEM** ✅ (validada ao vivo, São Roque). Faixas
    suave/média/alta + **flag legal ≥30%** (Lei 6.766/79 art. 3º §ún III) que entra na união do
    aproveitável (`declividade_vedada`). Fonte **keyless**: Copernicus GLO-30 Public COG na AWS
    (`/vsicurl`, sem chave), espelhando a 2.2; OpenTopography só fallback gated. Declividade em
    CRS métrico. `docs/fase-2.5-declividade.md`.
11. **Fase 3 — Pré-análise jurídica documental (dominial)** ✅. Lê matrícula/certidões (LLM
    PROPÕE ônus/averbações com referência ao ato R-x/Av-y; gate humano proposto→confirmado),
    faz **cross-check de área** (matrícula × KMZ da Fase 1) e **roll-up de risco** (domínio +
    alertas geo de 2.1/2.3/2.5). Reusa o motor da 1.8 (`ExtratorLUOS`→`ExtratorDocumento`),
    **sem credencial nova**. NUNCA afirma "imóvel livre". Não altera o aproveitável.
    `docs/fase-3-juridica-documental.md`.
12. **Fase 3.5 — Conformidade urbanística** ✅ (era a antiga "Fase 3"). Checklist da (zona,
    modalidade) contra a gleba com os índices da 1.8 que não entram no número (`frente_min`/
    `ca`/`taxa_ocupacao`/`doacao_split` + consistência split×total). Consumo puro, sem dado
    novo, sem LLM. Não altera o aproveitável. `docs/fase-3.5-conformidade.md`.
13. **Fase 4 — Financeira** ✅. Fluxo de caixa do empreendimento a partir dos lotes do motor +
    premissas declaradas (cada uma com proveniência declarado×default rotulado). VGV bruto/
    próprio, permuta (% VGV / lotes / compra), custos por bloco com curvas, comissão, marketing,
    **tributo como PARÂMETRO** (sem RET, default 5,93% rotulado "confirme com contador"), fluxo
    mês a mês + acumulado, **exposição máxima de caixa**, resultado nominal e margem. Aritmética
    PURA (sem LLM/rede); moeda formatada no backend (`_fmt`). Fronteira 4×5: monta o fluxo, não
    desconta. `docs/fase-4-financeira.md`.
14. Fase 5 — Econômica (VPL/TIR/TMA/payback/sensibilidade sobre o `fluxo` da 4). Pedido de spec
    em `docs/handoff-fase-5-pedido-spec.md`.
15. Fase 6 — Localização (enriquecimento IBGE).
16. Fase 7+ — Técnica / Operacional / Mercadológica / Política (guiadas).

Cada dimensão = **um endpoint no FastAPI + um card no Next.js**. Adiciona uma sem tocar nas outras.

**Transversal (futuro, decisão do operador em 2026-06-09):** login + **multi-tenant**. Hoje o
MVP é single-tenant: a persistência é global por chave de domínio (diretriz por `cod_ibge`,
ficha jurídica por `analise_id`). Quando entrar, re-chavear por tenant
(`{tenant}/{cod_ibge}`, `{tenant}/{analise_id}`) — código novo não deve assumir "global" como
verdade eterna.
