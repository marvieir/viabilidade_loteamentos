...
| 2026-06-05 | **DECISÃO do operador — modelo de aproveitável simplificado p/ TRIAGEM (muda contrato da Fase 1).** Ao validar a 2.2 na tela, o operador definiu: (a) **vias NÃO se descontam** (só se sabem no projeto urbanístico); (b) **doação NÃO se desconta** (depende da diretriz de cada prefeitura, que a plataforma ainda não carrega); (c) a **APP** (faixa da represa etc.) DEVE sair do aproveitável junto com a mata; (d) mostrar o **% sobre a gleba inteira**. Novo modelo: **área aproveitável = área total − UNIÃO(mata, APP curso d'água, APP massa d'água, faixa não-edificável, servidão LT)** — união geométrica (sem dupla contagem, `core/aproveitavel.consolidar`), pois mata ribeirinha é APP e verde ao mesmo tempo. **Nº de lotes urbano vira TETO** = aproveitável ÷ lote mínimo. Removidos do motor/endpoint: `aproveitamento_loteamento`/`_desmembramento`, as 3 "bases de doação", campos vias/doação/combinado/fator (e os valores-ouro da Aula 09, agora obsoletos). Endpoint injeta também `get_fonte_camadas` (APP). Schemas: `DescontosOut` (união + itens por tipo), `AproveitamentoOut` com `area_aproveitavel_m2`/`pct_sobre_total`/`n_lotes_teto`. Front: card reescrito (sem vias/doação; métrica de aproveitável + teto + breakdown). **70 testes verdes**; `tsc` limpo. | 2.2 |
| 2026-06-04 | **2.2 VALIDADA AO VIVO (operador, sem login) — regra confirmada.** `baixar_worldcover.py` rodou no Mac sem autenticar (tile S24W048 lido por `/vsicurl/`). Gleba **Terreno_Cachoeira (24,08 ha)**: árvores 57,9% / pasto 41,3% / água 0,7%. Com classes `{10,20,90,95}` → **verde 13,77 ha (57,2%)**. **Decisão de produto**: pasto/campo (30) NÃO entra no desconto (é aproveitável; a mata é a parte com restrição) — padrão `CLASSES_VERDE_PADRAO` confirmado, sem mudança de código. Valor-ouro fixado na spec. Falta só o ok visual final do operador no satélite. | 2.2 |
| 2026-06-04 | **2.2 — fonte trocada p/ ESA WorldCover (SEM login) após GEE bloquear o operador.** O `earthengine authenticate` esbarrou em bloqueio de login Google. Fonte PADRÃO passou a ser **ESA WorldCover (10 m, 2021)** — COG público na AWS Open Data, lido por `rasterio`/`/vsicurl/` SEM autenticação. Novo `scripts/baixar_worldcover.py` lê só a janela da gleba por HTTP → recorte local. `FonteVegetacaoRaster` ganhou `fonte` configurável e default `CLASSES_VERDE_PADRAO = {10,20,90,95}` (WorldCover); envs preferidas `VEGETACAO_RASTER_PATH`/`VEGETACAO_CLASSES_VERDE` (compat com as `MAPBIOMAS_*`). MapBiomas/GEE segue como alternativa opcional. Leitura remota NÃO testável aqui (egress 403) — amostragem coberta por `test_vegetacao_raster.py`. | 2.2 |
