...
```
Concluídas (validadas):  1 → 1.5 → 1.7 → 2.1 → 2.2 → 2.3 → 1.8 → 2.5 → 3 → 3.5 → 4 → 4.1
Próximas (nesta ordem):  5 → 6 → 7+                      (1.6* parado: falta amostra)
```
...
- **5 — Econômica (PRÓXIMA)** — TIR/MTIR/VPL/ROE/payback/exposição a VP sobre o `fluxo` da 4/4.1.
  **Gabarito pronto:** aba `IND. FINANCEIRO E TECNICO CP 11` da TIV 5.0 (caso real São Roque).
...
14. **Fase 4.1 — Financeira corretiva + PRICE (PRÓXIMA)**. Correções de UX (inadimplência
    ...
    comissão sobre recebimento. Valores-ouro calculados (pmt 1.890,78; rec. fin. agregada
    2.844.668,32). `docs/fase-4.1-financeira-price.md`.
15. **Fase 5 — Econômica**. TIR/MTIR/VPL/lucratividade/ROE/payback/exposição a VP sobre o
    `fluxo` da 4/4.1. **Gabarito: aba `IND. FINANCEIRO E TECNICO CP 11` da TIV 5.0** (caso
    São Roque: TIR 121% a.a., VPL 42,2M, payback 28, exp. máx −6,8M no mês 17).
16. Fase 6 — Localização (enriquecimento IBGE).
