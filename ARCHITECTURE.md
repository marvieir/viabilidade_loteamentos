...
```
Concluídas (validadas):  1 → 1.5 → 1.7 → 2.1 → 2.2 → 2.3 → 1.8 → 2.5 → 3 → 3.5
Próximas (nesta ordem):  1.6* → 4 → 5 → 6 → 7+
```
...
- **4 (próxima) — Financeira** — consome lotes do motor; é onde a planilha do curso entra.
  **A especificar:** pedido de spec em `docs/handoff-fase-4-pedido-spec.md`.
...
13. Fase 4 — Financeira (consome lotes do motor; fluxo de caixa do empreendimento — a
    planilha do curso entra aqui, **com as correções da §9**). Pedido de spec em
    `docs/handoff-fase-4-pedido-spec.md`.
14. Fase 5 — Econômica (consome fluxo da financeira).
